...
import EditClient from "./edit-company";
import DeleteClient from "./delete-company";
import { useSelector } from "react-redux";
const { Text } = Typography;

const CompanyDetail = ({ data, goBack }) => {
  ...
  const userDropdown = (record) => (
    <Menu className="!py-3 border border-[#DBDBDB] rounded-[5px] !shadow-none">
      <Menu.Item key="0">
        <EditClient data={record} />
      </Menu.Item>
      <Menu.Item key="1">
        <DeleteClient goBack={goBack} data={record} />
      </Menu.Item>
    </Menu>
  );
  return (
    <>
      <Breadcrumb
        ...
        items={[
          ...
          {
            title: <a href="">Customer Detail</a>,
          },
        ]}
      />
